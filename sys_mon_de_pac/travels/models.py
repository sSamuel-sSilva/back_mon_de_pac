...
class TravelBooking(models.Model):
    ...
    travel = models.ForeignKey(Travel, on_delete=models.DO_NOTHING, verbose_name='Viagem')
    ...
    date = models.DateField(verbose_name='Data da Solicitação', auto_now_add=True)
    time = models.TimeField(verbose_name='Hora da Solicitação', auto_now_add=True)

    observations = models.TextField(max_length=256, verbose_name="Observações", null=True, blank=True)
...
class CancelTravelBookingTicket(models.Model):
    ...
    owner = models.ForeignKey(CustomUser, on_delete=models.DO_NOTHING, verbose_name="Criador do Ticket")
    travel_booking = models.ForeignKey(TravelBooking, on_delete=models.CASCADE, verbose_name="Solicitação")
    reason = models.IntegerField(choices=Reason.choices, verbose_name="Razão do Cancelamento")
    date = models.DateField(verbose_name='Data do Cancelamento', auto_now_add=True)
    time = models.TimeField(verbose_name='Hora do Cancelamento', auto_now_add=True)

    class Meta:
        verbose_name = "Ticket de Cancelamento"
        verbose_name_plural = "Tickets de Cancelamento"


class CancelTravelTicket(models.Model):
    class Reason(models.IntegerChoices):
        MOTIVO = 0 ("motivo 0")
        MOTIVO1 = 1 ("motivo 1")
        MOTIVO2 = 2 ("motivo 2")
    ...
    reason = models.TextField(max_length=256, verbose_name="Razão do Cancelamento")
    date = models.DateField(verbose_name='Data do Cancelamento', auto_now_add=True)
    time = models.TimeField(verbose_name='Hora do Cancelamento', auto_now_add=True)

    class Meta:
        verbose_name = "Ticket de Cancelamento de Viagem"
        verbose_name_plural = "Tickets de Cancelamento de Viagem"
